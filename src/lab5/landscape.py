...
def get_elevation(size):
    ...
    elevation = np.array([[noise([i/xpix, j/ypix])
                         for j in range(ypix)] for i in range(xpix)])
...
def elevation_to_rgba(elevation):
    ...
    landscape = np.array([colormap(elevation[i, j])[0:3] for i in range(
        xpix) for j in range(ypix)]).reshape(xpix, ypix, 3)*255
    landscape = landscape.astype('uint8')
    return landscape


def get_landscape(size): return elevation_to_rgba(get_elevation(size))
