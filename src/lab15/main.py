...
# Also using the chat gpt api to tell the similarity between the two sentences which gives me a number from 0 - 1, where i tak the avarger between the three methods to get the most accurate
# results to determine the similarity between the two sentences
...
sentences = sentences = [
    ...
    ["I like bananas and pizza", "I love going to a long bike rides"]
]
...
def getSimilarityWithChatGPT(sentence1, sentence2):
    completion = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "user",
                "content": "Give me simaity score between [1,0] for these and only give me a number not explaination: ["+sentence1+","+sentence2+"]"}
        ]
    )
...
def calculate_similarity(text1, text2):
    ...
    sentence_scores.append(getSimilarityWithChatGPT(text1, text2))
